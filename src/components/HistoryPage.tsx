...
import { getHistory, type DailyTodos } from '@/utils/storage';
...
const HistoryPage: React.FC<HistoryPageProps> = ({ onBack }) => {
  ...
  const getCompletionStats = (todos: DailyTodos['todos']) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-100 via-pink-100 to-blue-100 p-4">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="text-center">
            <h1 className="text-4xl font-bold gradient-text flex items-center space-x-2">
              <Calendar className="w-8 h-8 text-purple-500 sparkle" />
        ...
        {history.length === 0 ? (
          <Card className="glass-effect border-0 shadow-xl text-center py-12">
            <CardContent>
              <Sparkles className="w-16 h-16 text-purple-400 mx-auto mb-4 bounce-gentle" />
          ...
          <div className="grid gap-6">
            {history.map((day, index) => {
              ...
              return (
                <Card
                  key={day.date}
                  className="glass-effect border-0 shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:scale-[1.02]"
                >
